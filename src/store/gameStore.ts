...
interface GameSessionState {
  ...
  enterLobby: (roomId: string, playerId: string, playerName?: string) => void;
  leaveLobby: () => void;
  roomFx: RoomFxState;
...
export const useGameStore = create<GameSessionState>()(
  persist(
    (set, get) => ({
      ...
      enterLobby: (roomId, playerId, playerName) =>
        set({
          activeRoomId: roomId,
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state, error) => {
        if (error) console.error("Failed to rehydrate game store", error);
